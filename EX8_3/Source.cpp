...
void DisplayBoard(vector<int> &Row)
{
	for (int row = 0; row < NUMBER_ROWS; ++row) {
		if (Row[row] == UNASSIGN_VALUE) {
			cout << "ERROR!  Row has not been assigned " << row << endl;
		}
...
*/
int i = 2;
bool PlaceQueen(int column, int rowIn = 0) {
	//DisplayBoard(RowPlacement);
	if (column == NUMBER_COLS+1) { // Base Condition
		...
		return true; // Successfully assigned all column queens in the board.  We can just return true.
			...
			for (int row = rowIn; row < RowPlacement.size(); ++row) {
				if (column == NUMBER_COLS) {
					DisplayBoard(RowPlacement);
					PlacementSolutions.push_back(RowPlacement);
					column = column - i;
					cout << "column is: " << column << " Row: " << RowPlacement[column] + 1 << endl;
				...
				if (RowPlacement[row] != UNASSIGN_VALUE || RowPlacement[row]>column)
					continue;
